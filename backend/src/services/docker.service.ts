...
export class DockerService {
    ...
    private validateDockerfile(dockerfile: string, repoDir: string): { valid: boolean; missingFiles: string[] } {
        ...
        const copyMatches = dockerfile.matchAll(/COPY\s+([^\s]+)\s+([^\s]+)/g);
...
- Respond ONLY with the Dockerfile content, no explanation or markdown.`;
...
CMD ["nginx", "-g", "daemon off;"]`;
    ...
    async buildImage(dockerfile: string, repoDir: string): Promise<void> {
        console.log('Building Docker image in directory:', repoDir);
